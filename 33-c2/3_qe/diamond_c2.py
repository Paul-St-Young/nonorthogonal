...
def apply_machine_settings(machine):
  ...
  if machine == 'quartz':
    ...
    dmc_job = Job(nodes=1,cores=36,hours=4,app='/g/g91/yang41/soft/master_qmcpack/build/bin/qmcpack_comp')
...
def gamma_dmc_input(p2q,dmc_job,system):
  ...
  correlation_time = 1.0 # steps = [correlation time]/[time step]
  ...
  vmc_input = obj(
    ...
    substeps    =   1,
    timestep    = 1.0,
    walkers     = 144, # vmc walkers per MPI group
