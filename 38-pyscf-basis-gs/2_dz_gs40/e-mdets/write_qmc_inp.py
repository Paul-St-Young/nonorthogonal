...
def jopt_inp(idet,h5_loc,opt_inp,pseudos,fftgrid):
  ...
  wf_node.find('.//sposet_builder').set('precision','float')
...
if __name__ == '__main__':
  ...
  idet = 50 # number of determinants in wavefunction

  proj_id = 'mno'
  pseudo_dir = '~/pseudo'
  pseudos = {'Mn':'Mn.BFD.xml','O':'O.BFD.xml'}
  fftgrid = np.array([100,100,100])
  h5_loc  = '../d-dets/pyscf2qmcpack.h5'
  vmc_inp = 'vmc.xml'
  opt_inp = 'opt.xml'
  dmc_inp = 'dmc.xml'
  coeff_fname_fmt = '../c-afqmc/afcoeff{ndet:d}.dat'

  # get simulation cell (nup & ndn)
  chkfile = '../a-uhf/gs40.h5'
  ...
  nvmc_walkers = 16*4 # 64 walkers/MPI group
  ndmc_walkers = 9216 # 16 nodes * 32 cores/node * 8 walkers/core
  nloop = 8
  ...
  jopt_inp(idet,h5_loc,opt_inp,pseudos,fftgrid)
  assert False,'remove this line after optimization is done, i.e. %s must exist' % opt_jas_fname
