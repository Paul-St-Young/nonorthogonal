...
def step5_generate_qmcpack_wavefunction_file(detlist_fname,wf_h5_fname,nfill,mf):
  ...
  generate_pwscf_h5(mf.cell,gvecs,eig_df,h5_fname=wf_h5_fname)
...
def step5_1_single_det_h5s(detlist_fname,nfill,mf):
  ...
  for idet in range(ndet):
    ...
    generate_pwscf_h5(mf.cell,gvecs,eig_df,h5_fname='det%d.h5'%(idet))
  # end for ifile
# end def step5_1_single_det_h5s

def dump_molecular_orbitals(mf,nfill):
...
if __name__ == '__main__':
  ...
  nfill = 8            # number of occupied orbitals in each determinant
  ...
  check_points = {
    1:'dzv.h5',      # ~ 2 min using 72 threads
  ...
  if not os.path.isfile( check_points[1] ):
    ...
    #dump_molecular_orbitals(mf,nfill)
    print(mf.e_tot)
    print(datetime.now())
  # end if
