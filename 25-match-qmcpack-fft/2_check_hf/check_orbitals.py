...
if __name__ == '__main__':
  ...
  moR = np.loadtxt('../1_gs16/moR%d.dat' % istate).view(complex).reshape(grid_shape)
  print (qafter_fftbox.real/moR.real).mean()
  import matplotlib.pyplot as plt
  fig,ax = plt.subplots(2,1)
  ax[0].plot( moR.real.flatten() )
  ax[0].plot( qafter_fftbox.real.flatten() )
  ax[1].plot( moR.imag.flatten() )
  ax[1].plot( qafter_fftbox.imag.flatten() )
  plt.show()
