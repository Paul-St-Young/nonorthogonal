...
def run_pyscf(xsf_file,gs,nz=2,verbose=4,chkfile_name='bfd.h5',exxdiv=None):
  ...
  # initialize density matrix to AFM order
  #from pyscf.scf.uhf import init_guess_by_minao #init_guess_by_minao(cell)
  #init_dm = mf.get_init_guess(key='minao')
  #dm = modify_1rdm(init_dm,[12,46])
...
if __name__ == '__main__':
  ...
  #ewald_corr = -7.458272 # ewald correction from a-hf, !!!! wrong, see h-gs32-ewald
  ...
  print(datetime.now())
  print(' HF done')
  #print(' SCF energy = %f, with ewald = %f'%(mf.e_tot,mf.e_tot+ewald_corr) )
