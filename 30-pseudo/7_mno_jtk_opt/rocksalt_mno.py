...
def apply_machine_settings(machine,run_name):
  ...
  if run_name not in ['bfd','hgh','opt']:
  ...
  if machine == 'quartz':
    scf_job = Job(nodes=1,minutes=30,queue='pdebug')
    p2q_job = Job(nodes=1,serial=True,minutes=5,queue='pdebug')
    dmc_job = Job(nodes=1,minutes=30,app='/g/g91/yang41/soft/master_qmcpack/build/bin/qmcpack_comp',queue='pdebug')
  ...
  if run_name == 'bfd':
    ...
    dft_pseudos = ['Mn.opt.upf','O.BFD.upf']
    qmc_pseudos = None
  # end if
